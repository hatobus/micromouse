...
extern void straight(int len, int tar_speed);
extern void straight_for_search(int len, int tar_speed);
...
void search_lefthand(void)
{
	...
	straight(HALF_SECTION,SEARCH_SPEED);			//まず、半区画進む
		
	while(1)
	{
		if(sen_l.is_wall == false)			//左に壁がなければ左に進む
		{
			straight(HALF_SECTION,0);		//半区画進んで
			rotate(left,1);				//左に曲がって
			straight(HALF_SECTION,SEARCH_SPEED);	//半区画進む
		}
		else if( (sen_fl.is_wall == false) && (sen_fr.is_wall == false) )	//前に壁がなければ前に進む
		{
			straight(SECTION,SEARCH_SPEED);		//一区画進む
		}
		else if(sen_r.is_wall == false)			//右に壁がなければ右に進む
		{
			straight(HALF_SECTION,0);		//半区画進む
...
//引数gx,gyに向かって足立法で迷路を探索する
	...
	switch(get_nextdir(gx,gy,MASK_SEARCH,&glob_nextdir))			//次に行く方向を戻り値とする関数を呼ぶ
	{
		...
		case rear:
			rotate(left,2);						//後ろに回って
			straight(HALF_SECTION,SEARCH_SPEED);			//半区画進む
			break;
	}
		r_accel = accel= SEARCH_ACCEL;					//加速度を設定
	...
	while((mypos.x != gx) || (mypos.y != gy)){					//ゴールするまで繰り返す
		...
		switch(get_nextdir(gx,gy,MASK_SEARCH,&glob_nextdir))			//次に行く方向を戻り値とする関数を呼ぶ
		{
			...
			case right:
				straight_for_search(HALF_SECTION,0);			//半区画進んで
				rotate(right,1);					//右に曲がって
				straight(HALF_SECTION,SEARCH_SPEED);			//半区画進む
				break;
			...
			case rear:
				straight_for_search(HALF_SECTION,0);			//半区画進んで
				rotate(left,2);						//後ろを向いて
				straight(HALF_SECTION,SEARCH_SPEED);			//半区画進む
				break;
		}

		r_accel = accel = SEARCH_ACCEL;						//加速度
		con_wall.enable = true;							//壁制御を有効にする
		MOT_CWCCW_R = MOT_CWCCW_L = MOT_FORWARD;				//前方に進む
		step_r = step_l = 0;							//進んだ距離をカウントする変数をリセット
		MTU.TSTR.BIT.CST3 = MTU.TSTR.BIT.CST4 = 1;				//カウントスタート
	...
	set_wall(mypos.x,mypos.y);		//壁をセット
	straight_for_search(HALF_SECTION,0);	//ゴール.半区画進む
